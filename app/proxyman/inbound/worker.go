...
func (w *dsWorker) callback(conn internet.Connection) {
	...
	if err := w.proxy.Process(ctx, net.Network_UNIX, conn, dest); err != nil {
...
func (w *tcpWorker) callback(conn internet.Connection) {
	...
	ctx = session.ContextWithContent(ctx, content)
	if err := w.proxy.Process(ctx, net.Network_TCP, conn, dest); err != nil {
		newError("connection ends").Base(err).WriteToLog(session.ExportIDToError(ctx))
	}
	cancel()
	if err := conn.Close(); err != nil {
...
func (w *udpWorker) callback(b *buf.Buffer, source net.Destination, originalDest net.Destination) {
	...
	if !existing {
		...
		go func() {
			...
			ctx = session.ContextWithContent(ctx, content)
			if err := w.proxy.Process(ctx, net.Network_UDP, conn, dest); err != nil {
				newError("connection ends").Base(err).WriteToLog(session.ExportIDToError(ctx))
			}
			conn.Close()
			if !conn.inactive {
