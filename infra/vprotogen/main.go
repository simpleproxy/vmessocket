...
func getProjectProtocVersion(url string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("can not get the version of protobuf used in vmessocket project")
	}
...
func main() {
	...
	targetedVersion, err := getProjectProtocVersion("https://raw.githubusercontent.com/v2fly/vmessocket-core/HEAD/config.pb.go")
	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
...
Download it from https://github.com/protocolbuffers/protobuf/releases
    ...
    * Protobuf version used in vmessocket project: v%s
